/**
 * Licensed to the Apache Software Foundation (ASF) under one
 * or more contributor license agreements.  See the NOTICE file
 * distributed with this work for additional information
 * regarding copyright ownership.  The ASF licenses this file
 * to you under the Apache License, Version 2.0 (the
 * "License"); you may not use this file except in compliance
 * with the License.  You may obtain a copy of the License at
 *
 *   http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing,
 * software distributed under the License is distributed on an
 * "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY
 * KIND, either express or implied.  See the License for the
 * specific language governing permissions and limitations
 * under the License.
 */
package org.apache.pinot.broker.materializedview.strategy;

import java.util.ArrayList;
import java.util.HashSet;
import java.util.List;
import java.util.Map;
import java.util.Set;
import javax.annotation.Nullable;
import org.apache.pinot.broker.materializedview.MvMatchResult;
import org.apache.pinot.broker.materializedview.MvMatchUtils;
import org.apache.pinot.broker.materializedview.MvMetadataCache;
import org.apache.pinot.broker.materializedview.MvQueryShape;
import org.apache.pinot.broker.materializedview.equivalence.AggregationEquivalence;
import org.apache.pinot.broker.materializedview.equivalence.AggregationEquivalenceRegistry;
import org.apache.pinot.common.request.Expression;
import org.apache.pinot.common.request.ExpressionType;
import org.apache.pinot.common.request.Function;
import org.apache.pinot.common.request.PinotQuery;
import org.apache.pinot.common.utils.request.RequestUtils;
import org.apache.pinot.sql.parsers.CalciteSqlParser;


/**
 * Subsumption strategy for <b>aggregation</b> queries. The rewritten query
 * takes one of two forms depending on whether the MV's pre-computed column
 * values can be read directly:
 *
 * <ul>
 *   <li><b>Scan rewrite</b> — every user SELECT expression maps directly to
 *       an MV column whose stored value is the final answer (same function
 *       name, same GROUP BY granularity). GROUP BY is removed, HAVING is
 *       converted to WHERE, and SELECT becomes plain column references.</li>
 *   <li><b>Aggregation rewrite</b> — one or more SELECT expressions require
 *       function transformation (e.g. sketch raw bytes → cardinality), or
 *       the MV has finer GROUP BY granularity requiring roll-up. GROUP BY
 *       is retained, aggregation columns are wrapped with re-aggregation
 *       functions via {@link AggregationEquivalenceRegistry}, and HAVING
 *       is kept as HAVING.</li>
 * </ul>
 *
 * <p>Cost model:
 * <ul>
 *   <li>{@code 4.0} — scan rewrite without residual WHERE</li>
 *   <li>{@code 5.0} — scan rewrite with residual WHERE</li>
 *   <li>{@code 6.0} — aggregation rewrite without residual WHERE</li>
 *   <li>{@code 7.0} — aggregation rewrite with residual WHERE</li>
 * </ul>
 *
 * <p>Implementations should be stateless and thread-safe.
 */
public class AggregationSubsumptionStrategy extends AbstractSubsumptionStrategy {

  private static final double COST_SCAN = 4.0;
  private static final double COST_SCAN_WITH_RESIDUAL = 5.0;
  private static final double COST_REAGG = 6.0;
  private static final double COST_REAGG_WITH_RESIDUAL = 7.0;

  /**
   * Both user and MV must be aggregation-shaped queries.
   */
  @Override
  protected boolean acceptsShape(PinotQuery userQuery, PinotQuery mvQuery) {
    return MvQueryShape.classify(userQuery) == MvQueryShape.AGGREGATION
        && MvQueryShape.classify(mvQuery) == MvQueryShape.AGGREGATION;
  }

  /**
   * Accepts both equal and finer MV granularity: the user's GROUP BY column
   * set must be a subset of (or equal to) the MV's GROUP BY column set.
   */
  @Override
  protected boolean groupByMatches(PinotQuery userQuery, PinotQuery mvQuery) {
    List<Expression> userGroupBy = userQuery.getGroupByList();
    List<Expression> mvGroupBy = mvQuery.getGroupByList();

    if (userGroupBy == null || userGroupBy.isEmpty()
        || mvGroupBy == null || mvGroupBy.isEmpty()) {
      return false;
    }

    Set<Expression> userSet = new HashSet<>(userGroupBy);
    Set<Expression> mvSet = new HashSet<>(mvGroupBy);
    return mvSet.containsAll(userSet);
  }

  /**
   * Checks that every user SELECT expression can be resolved against the MV.
   * For each alias-stripped expression:
   * <ul>
   *   <li>Direct match in {@code mvProjectionMap} (equal granularity path)</li>
   *   <li>Equivalence-based match via {@link AggregationEquivalenceRegistry}
   *       (finer MV granularity path): the user's aggregation function and
   *       an MV entry share the same operands, and a registered equivalence
   *       rule links them</li>
   * </ul>
   */
  @Override
  protected boolean projectionSubsumes(List<Expression> userSelectList,
      Map<Expression, String> mvProjectionMap) {
    if (userSelectList == null || userSelectList.isEmpty()) {
      return false;
    }
    for (Expression expr : userSelectList) {
      Expression stripped = MvMatchUtils.stripAlias(expr);
      if (mvProjectionMap.containsKey(stripped)) {
        continue;
      }
      if (findEquivalentMvEntry(stripped, mvProjectionMap) == null) {
        return false;
      }
    }
    return true;
  }

  /**
   * Searches the MV projection map for an entry whose aggregation operands
   * match the user expression's operands, and for which a registered
   * {@link AggregationEquivalence} rule exists.
   *
   * @return a two-element array {@code [mvColumnName, AggregationEquivalence]}
   *         if found, or {@code null} if no equivalence applies
   */
  @Nullable
  private static Object[] findEquivalentMvEntry(Expression userExpr,
      Map<Expression, String> mvProjectionMap) {
    Function userFunc = userExpr.getFunctionCall();
    if (userFunc == null) {
      return null;
    }
    String userFuncName = userFunc.getOperator();
    List<Expression> userOperands = userFunc.getOperands();

    for (Map.Entry<Expression, String> mvEntry : mvProjectionMap.entrySet()) {
      Expression mvExpr = mvEntry.getKey();
      Function mvFunc = mvExpr.getFunctionCall();
      if (mvFunc == null) {
        continue;
      }
      if (!operandsMatch(userOperands, mvFunc.getOperands())) {
        continue;
      }
      AggregationEquivalence rule =
          AggregationEquivalenceRegistry.findRule(userFuncName, mvFunc.getOperator());
      if (rule != null) {
        return new Object[]{mvEntry.getValue(), rule};
      }
    }
    return null;
  }

  // TODO: Current approach strips ALL literal operands before comparison. This
  //  tolerates broker-injected config params (e.g. log2m from
  //  handleHLLLog2mOverride) but may be too permissive — if a function has
  //  semantically significant literal operands (e.g. PERCENTILE(col, 50) vs
  //  PERCENTILE(col, 99)), stripping them would cause an incorrect match.
  //  For now this is acceptable because findEquivalentMvEntry only compares
  //  functions with DIFFERENT names (same-name functions are already handled
  //  by direct key lookup in projectionSubsumes), but this assumption should
  //  be revisited if the matching scope expands. A more principled solution
  //  could be moving MV matching before broker overrides in the request
  //  handler pipeline, or letting each AggregationEquivalence rule define
  //  its own operand matching semantics.
  private static boolean operandsMatch(@Nullable List<Expression> a, @Nullable List<Expression> b) {
    if (a == null && b == null) {
      return true;
    }
    if (a == null || b == null) {
      return false;
    }
    return extractColumnOperands(a).equals(extractColumnOperands(b));
  }

  private static List<Expression> extractColumnOperands(List<Expression> operands) {
    List<Expression> result = new ArrayList<>(operands.size());
    for (Expression expr : operands) {
      if (expr.getType() != ExpressionType.LITERAL) {
        result.add(expr);
      }
    }
    return result;
  }

  /**
   * Validates that all columns referenced in the residual filter are GROUP BY
   * columns of the MV. Non-GROUP-BY columns have been aggregated away and
   * cannot be filtered after the fact.
   */
  @Override
  protected boolean validateResidual(@Nullable Expression residualFilter, PinotQuery mvQuery) {
    if (residualFilter == null) {
      return true;
    }

    List<Expression> mvGroupBy = mvQuery.getGroupByList();
    if (mvGroupBy == null || mvGroupBy.isEmpty()) {
      return false;
    }

    Set<String> groupByColumnNames = new HashSet<>(mvGroupBy.size());
    for (Expression gbExpr : mvGroupBy) {
      groupByColumnNames.addAll(MvMatchUtils.collectReferencedColumns(gbExpr));
    }

    Set<String> residualColumns = MvMatchUtils.collectReferencedColumns(residualFilter);
    return groupByColumnNames.containsAll(residualColumns);
  }

  /**
   * Validates that all expressions referenced in the ORDER BY clause can be
   * resolved — either via direct lookup in the MV projection map or via
   * equivalence-based matching for aggregation expressions.
   */
  @Override
  protected boolean orderByCompatible(PinotQuery userQuery, PinotQuery mvQuery,
      Map<Expression, String> mvProjectionMap) {
    List<Expression> orderByList = userQuery.getOrderByList();
    if (orderByList == null || orderByList.isEmpty()) {
      return true;
    }
    for (Expression orderByExpr : orderByList) {
      Expression inner = CalciteSqlParser.removeOrderByFunctions(orderByExpr);
      if (!isResolvable(inner, mvProjectionMap)) {
        return false;
      }
    }
    return true;
  }

  /**
   * Validates that all non-literal sub-expressions within the HAVING clause
   * can be resolved — either via the MV projection map directly, or via
   * equivalence-based matching for aggregation expressions.
   */
  @Override
  protected boolean havingCompatible(PinotQuery userQuery, PinotQuery mvQuery,
      Map<Expression, String> mvProjectionMap) {
    Expression having = userQuery.getHavingExpression();
    if (having == null) {
      return true;
    }
    return allReferencesResolvableWithEquivalence(having, mvProjectionMap);
  }

  /**
   * Like {@link MvMatchUtils#allReferencesResolvable} but also considers
   * equivalence-based matching for aggregation function expressions.
   */
  private boolean allReferencesResolvableWithEquivalence(Expression expr,
      Map<Expression, String> mvProjectionMap) {
    if (isResolvable(expr, mvProjectionMap)) {
      return true;
    }
    if (expr.getType() == ExpressionType.LITERAL) {
      return true;
    }
    if (expr.getType() == ExpressionType.IDENTIFIER) {
      return false;
    }
    if (expr.getFunctionCall() != null && expr.getFunctionCall().getOperands() != null) {
      for (Expression operand : expr.getFunctionCall().getOperands()) {
        if (!allReferencesResolvableWithEquivalence(operand, mvProjectionMap)) {
          return false;
        }
      }
      return true;
    }
    return false;
  }

  /**
   * Checks whether a single expression can be resolved against the MV
   * projection map — either by direct key lookup or equivalence matching.
   */
  private static boolean isResolvable(Expression expr, Map<Expression, String> mvProjectionMap) {
    if (mvProjectionMap.containsKey(expr)) {
      return true;
    }
    return findEquivalentMvEntry(expr, mvProjectionMap) != null;
  }

  /**
   * Builds the rewritten query targeting the MV table.
   *
   * <p>The scan path is chosen only when <b>both</b> conditions hold:
   * <ol>
   *   <li>The MV's GROUP BY granularity is identical to the user query's.</li>
   *   <li>Every user SELECT expression directly maps to an MV column with no
   *       function transformation needed.</li>
   * </ol>
   *
   * <p>Although this is an aggregation strategy, meeting both conditions
   * means the MV table already contains fully materialized results — each
   * row is the final answer for its group. The rewrite therefore degenerates
   * into a simple scan: GROUP BY is removed, HAVING is converted to WHERE,
   * and SELECT expressions become plain column references. This avoids a
   * redundant re-aggregation pass over single-row groups.
   *
   * <p>In all other cases (finer MV granularity, or any SELECT expression
   * requiring equivalence-based function transformation such as
   * {@code DISTINCTCOUNTHLL} vs {@code DISTINCTCOUNTRAWHLL}), the
   * aggregation path is used.
   */
  @Override
  protected MvMatchResult buildResult(PinotQuery userQuery, MvMetadataCache.MvCacheEntry candidateEntry,
      @Nullable Expression residualFilter, Map<Expression, String> mvProjectionMap, boolean filtersEqual) {
    boolean canScan = !isFinerMvGranularity(userQuery, candidateEntry)
        && allExpressionsDirectMatch(userQuery.getSelectList(), mvProjectionMap);
    if (canScan) {
      return buildScanResult(userQuery, candidateEntry, residualFilter, mvProjectionMap);
    }
    return buildAggregationResult(userQuery, candidateEntry, residualFilter, mvProjectionMap);
  }

  // -----------------------------------------------------------------------
  //  Scan rewrite — direct column substitution, no re-aggregation
  // -----------------------------------------------------------------------

  private MvMatchResult buildScanResult(PinotQuery userQuery, MvMetadataCache.MvCacheEntry candidateEntry,
      @Nullable Expression residualFilter, Map<Expression, String> mvProjectionMap) {
    PinotQuery rewritten = userQuery.deepCopy();
    rewritten.getDataSource().setTableName(candidateEntry.getMetadata().getMvTableNameWithType());

    rewritten.setSelectList(MvMatchUtils.rewriteSelectList(userQuery.getSelectList(), mvProjectionMap));
    rewritten.setGroupByList(null);

    Expression remappedHaving = null;
    Expression originalHaving = userQuery.getHavingExpression();
    if (originalHaving != null) {
      remappedHaving = MvMatchUtils.remapExpression(originalHaving, mvProjectionMap);
    }
    rewritten.setHavingExpression(null);

    Expression remappedResidual = null;
    if (residualFilter != null) {
      remappedResidual = MvMatchUtils.remapExpression(residualFilter, mvProjectionMap);
    }

    rewritten.setFilterExpression(mergeFilters(remappedHaving, remappedResidual));

    remapOrderBy(rewritten, userQuery, mvProjectionMap);

    double cost = (residualFilter != null) ? COST_SCAN_WITH_RESIDUAL : COST_SCAN;
    return new MvMatchResult(rewritten, candidateEntry.getMetadata(), cost);
  }

  // -----------------------------------------------------------------------
  //  Aggregation rewrite — re-aggregation with GROUP BY retained
  // -----------------------------------------------------------------------

  private MvMatchResult buildAggregationResult(PinotQuery userQuery, MvMetadataCache.MvCacheEntry candidateEntry,
      @Nullable Expression residualFilter, Map<Expression, String> mvProjectionMap) {
    PinotQuery rewritten = userQuery.deepCopy();
    rewritten.getDataSource().setTableName(candidateEntry.getMetadata().getMvTableNameWithType());

    // SELECT: dimension columns remapped, aggregation columns re-wrapped
    rewritten.setSelectList(
        buildReAggSelectList(userQuery.getSelectList(), mvProjectionMap));

    // GROUP BY: retain user's GROUP BY columns, remapped to MV column names
    List<Expression> userGroupBy = userQuery.getGroupByList();
    List<Expression> remappedGroupBy = new ArrayList<>(userGroupBy.size());
    for (Expression gbExpr : userGroupBy) {
      String mvCol = mvProjectionMap.get(gbExpr);
      remappedGroupBy.add(RequestUtils.getIdentifierExpression(mvCol));
    }
    rewritten.setGroupByList(remappedGroupBy);

    // HAVING: kept as HAVING (re-aggregation produces new aggregate values)
    Expression originalHaving = userQuery.getHavingExpression();
    if (originalHaving != null) {
      rewritten.setHavingExpression(
          remapExpressionWithEquivalence(originalHaving, mvProjectionMap));
    } else {
      rewritten.setHavingExpression(null);
    }

    // Residual WHERE: only references dimension (GROUP BY) columns
    Expression remappedResidual = null;
    if (residualFilter != null) {
      remappedResidual = MvMatchUtils.remapExpression(residualFilter, mvProjectionMap);
    }
    rewritten.setFilterExpression(remappedResidual);

    // ORDER BY: remap with equivalence awareness
    if (userQuery.getOrderByList() != null && !userQuery.getOrderByList().isEmpty()) {
      List<Expression> remappedOrderBy = new ArrayList<>(userQuery.getOrderByList().size());
      for (Expression orderByExpr : userQuery.getOrderByList()) {
        remappedOrderBy.add(remapExpressionWithEquivalence(orderByExpr, mvProjectionMap));
      }
      rewritten.setOrderByList(remappedOrderBy);
    }

    double cost = (residualFilter != null) ? COST_REAGG_WITH_RESIDUAL : COST_REAGG;
    return new MvMatchResult(rewritten, candidateEntry.getMetadata(), cost);
  }

  /**
   * Builds the SELECT list for a finer-MV rewrite. Dimension columns become
   * plain MV identifiers; aggregation columns are wrapped with their
   * re-aggregation function via the equivalence framework.
   */
  private List<Expression> buildReAggSelectList(List<Expression> userSelectList,
      Map<Expression, String> mvProjectionMap) {
    List<Expression> result = new ArrayList<>(userSelectList.size());
    for (Expression expr : userSelectList) {
      Expression stripped = MvMatchUtils.stripAlias(expr);
      String userAlias = MvMatchUtils.extractUserAlias(expr);
      Expression rewritten;

      if (mvProjectionMap.containsKey(stripped)
          && stripped.getFunctionCall() == null) {
        // Dimension column: direct MV column reference
        rewritten = RequestUtils.getIdentifierExpression(mvProjectionMap.get(stripped));
      } else {
        // Aggregation column: apply re-aggregation via equivalence
        rewritten = rewriteAggregationExpression(stripped, mvProjectionMap);
      }

      if (userAlias != null) {
        rewritten = RequestUtils.getFunctionExpression("as", rewritten,
            RequestUtils.getIdentifierExpression(userAlias));
      }
      result.add(rewritten);
    }
    return result;
  }

  /**
   * Rewrites a single aggregation expression for finer-MV mode. Looks up the
   * equivalence rule and applies it to produce the re-aggregation expression.
   */
  private Expression rewriteAggregationExpression(Expression stripped,
      Map<Expression, String> mvProjectionMap) {
    // Try direct match first (e.g. equal-granularity dimension that happens
    // to have a function call like DATETIMECONVERT)
    if (mvProjectionMap.containsKey(stripped)) {
      String mvCol = mvProjectionMap.get(stripped);
      Function func = stripped.getFunctionCall();
      if (func != null) {
        AggregationEquivalence rule =
            AggregationEquivalenceRegistry.findRule(func.getOperator(), func.getOperator());
        if (rule != null) {
          return rule.rewrite(stripped, mvCol);
        }
      }
      return RequestUtils.getIdentifierExpression(mvCol);
    }

    // Fuzzy match via equivalence
    Object[] match = findEquivalentMvEntry(stripped, mvProjectionMap);
    if (match != null) {
      String mvCol = (String) match[0];
      AggregationEquivalence rule = (AggregationEquivalence) match[1];
      return rule.rewrite(stripped, mvCol);
    }

    // Should not reach here if projectionSubsumes passed
    throw new IllegalStateException(
        "Cannot rewrite aggregation expression: " + stripped);
  }

  /**
   * Remaps an expression tree (HAVING / ORDER BY) for finer-MV mode.
   * Aggregation sub-expressions are replaced with their re-aggregation
   * form; dimension identifiers are replaced with MV column names.
   */
  private Expression remapExpressionWithEquivalence(Expression expr,
      Map<Expression, String> mvProjectionMap) {
    // Direct match as dimension column
    if (mvProjectionMap.containsKey(expr) && expr.getFunctionCall() == null) {
      return RequestUtils.getIdentifierExpression(mvProjectionMap.get(expr));
    }

    // Aggregation expression: try direct then fuzzy match
    if (expr.getFunctionCall() != null) {
      if (mvProjectionMap.containsKey(expr)) {
        return rewriteAggregationExpression(expr, mvProjectionMap);
      }
      Object[] match = findEquivalentMvEntry(expr, mvProjectionMap);
      if (match != null) {
        return ((AggregationEquivalence) match[1]).rewrite(expr, (String) match[0]);
      }
    }

    // Recurse into function operands (comparisons, logical operators, etc.)
    if (expr.getFunctionCall() != null && expr.getFunctionCall().getOperands() != null) {
      Function func = expr.getFunctionCall();
      List<Expression> original = func.getOperands();
      List<Expression> remapped = new ArrayList<>(original.size());
      boolean changed = false;
      for (Expression operand : original) {
        Expression result = remapExpressionWithEquivalence(operand, mvProjectionMap);
        remapped.add(result);
        if (result != operand) {
          changed = true;
        }
      }
      if (changed) {
        return RequestUtils.getFunctionExpression(func.getOperator(), remapped);
      }
    }

    return expr;
  }

  // -----------------------------------------------------------------------
  //  Rewrite mode detection
  // -----------------------------------------------------------------------

  /**
   * Returns {@code true} if the MV's GROUP BY is a strict superset of the
   * user's GROUP BY (finer granularity).
   */
  private static boolean isFinerMvGranularity(PinotQuery userQuery,
      MvMetadataCache.MvCacheEntry candidateEntry) {
    PinotQuery mvQuery = candidateEntry.getCompiledQuery();
    Set<Expression> userSet = new HashSet<>(userQuery.getGroupByList());
    Set<Expression> mvSet = new HashSet<>(mvQuery.getGroupByList());
    return !userSet.equals(mvSet) && mvSet.containsAll(userSet);
  }

  /**
   * Returns {@code true} if every user SELECT expression (alias-stripped)
   * directly exists as a key in the MV projection map — no equivalence
   * matching required.
   */
  private static boolean allExpressionsDirectMatch(List<Expression> userSelectList,
      Map<Expression, String> mvProjectionMap) {
    for (Expression expr : userSelectList) {
      Expression stripped = MvMatchUtils.stripAlias(expr);
      if (!mvProjectionMap.containsKey(stripped)) {
        return false;
      }
    }
    return true;
  }

  private static void remapOrderBy(PinotQuery rewritten, PinotQuery userQuery,
      Map<Expression, String> mvProjectionMap) {
    List<Expression> orderByList = userQuery.getOrderByList();
    if (orderByList != null && !orderByList.isEmpty()) {
      List<Expression> remappedOrderBy = new ArrayList<>(orderByList.size());
      for (Expression orderByExpr : orderByList) {
        remappedOrderBy.add(MvMatchUtils.remapExpression(orderByExpr, mvProjectionMap));
      }
      rewritten.setOrderByList(remappedOrderBy);
    }
  }

  /**
   * Merges two optional filter expressions into a single expression using AND.
   * Returns {@code null} if both inputs are null.
   */
  @Nullable
  private static Expression mergeFilters(@Nullable Expression filter1, @Nullable Expression filter2) {
    if (filter1 == null) {
      return filter2;
    }
    if (filter2 == null) {
      return filter1;
    }
    List<Expression> conjuncts = new ArrayList<>(2);
    conjuncts.add(filter1);
    conjuncts.add(filter2);
    return MvMatchUtils.buildAndExpression(conjuncts);
  }
}
